...
def stir_the_coffee():
	...
	iiwa_joint_move(0,0,0,0,0,pi/2,0) ; 
	carthesian_movement_relative(0,0,-0.02)
	carthesian_movement_relative(0,0,0.02)
	carthesian_movement_relative(0,0,-0.02)
	carthesian_movement_relative(0,0,0.02)
...
def turn_on_water_boiler():
	pass

def get_coffeepowder(number_scoops):
	print "go to coffee_spoon"
	for i in range(number_scoops):
...
def pour_water():
	boiler.go_to_location()
	pick_up_object()
	mug_place.go_to_location()
	iiwa_joint_move(0,0,0,0,0,0,pi/3) 
	iiwa_joint_move(0,0,0,0,0,0,-pi/3) 
	boiler.go_to_location()
	place_object()
...
##########
def make_coffee_task():
	turn_on_water_boiler()		#Turn on water boiler
	mug.go_to_location()		#Go to the mug location
	pick_up_object()			#pick up object
	mug_place.go_to_location()	#Go to the desired mug location
	place_object()				#place object
	get_coffeepowder(2) 		#Take 2 scopes
	pour_water()				#pour water in
	stir_the_coffee() 			#Stif the coffee
